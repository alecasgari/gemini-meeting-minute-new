...
@app.route("/meeting/<int:meeting_id>/print")
@login_required
def print_meeting(meeting_id):
    meeting = Meeting.query.get_or_404(meeting_id)
    if meeting.author != current_user:
        abort(403)
    ...
    # Get font URLs for web display
    fa_regular_url = None
    fa_bold_url = None
    fa_regular_format = None
    fa_bold_format = None
    if selected_fa_files:
        if 'regular' in selected_fa_files:
            fa_regular_url = url_for('static', filename=f"fonts/{selected_fa_files['regular']}")
    ...
    # Get logo URL
    logo_url = url_for('static', filename=f"images/{logo_filename}") if os.path.exists(logo_fs_path) else None
